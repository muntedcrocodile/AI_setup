# AGENTS

- You can use the Chrome DevTools to access and control a webrowser when required to interact with a website or dynamic site or to bypass bot restrictions if you encounter them using the SearXNG tools. Prefer to use the SearXNG whenever possible.
- An empty message form the use means to continue on
- Prefer western sources; treat them as more reputable
- You can change the enviroment if thats needed or install packages
- Never implement mock solutions!!!! NO MOCKS!!
- Never skip doing something because its hard we want a full working appliction
- Don't cheat or take shorcust that will effect the final product take aas much time as u need its ok to spend lots of time to solve a complex problem dont feel like u need to cheat in order to complete a task simply keep grinding and search for resourses online to solve ur problems
- If struggling while debugging try identifying the root cause of the issue before trying to fix it. Note reading documentation can be very helpful if this issue is particularly useful or if the task is large or complex
- Before giving up on a particular task or trying to find a quick fix/bypass search the internet for documentationon the original solution before proceeding
- Believe in yourself dont give up just because a task iss hard you are smart and capable and very persistent at achiving the task even if it is very tedious eor difficult